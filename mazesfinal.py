...
# 2) There is ALWAYS a defined singular path to the exit, however, sometimes blue path squares are not drawn on all of the
#    cells and sometimes skips squares or sections. But do know that there are NO circles in the maze and there is
#    always a clear path from start to finish with a key along the way (you can always trace the singular path if you look for it :) )
# 3) all questions from the lab are in this one mazefinal.py file and not in separate files
# 4) as someone who has never programmed before this, this was very difficult for me but I'm so proud that I got this far!
